...
검증일: 2026-09-23. 이 회차는 `claim` 아크 페이즈 1이며 **판별선은 결정 세션의 낙착을 원장에
싣고 `claim` 아크를 여는 것**이다. **이 회차는 아크를 열기만 하고 닫지 않는다.**

**아크를 바꾼 첫 회차다.** 앞선 열여섯 회차가 전부 `charter` 아크 안에 있었고 그 아크가 앞
회차에서 닫혔다. 이 회차부터 `claim` 이며 Lean 진술은 하나도 세우지 않는다.

### 62-1. 빌드

세 파일 전량 통과이고 `sorry` 와 실패가 없다. `Build completed successfully (978 jobs)` 이며
error 0 warning 0 이다. **이 회차는 Lean 파일을 한 자리 건드렸고 그것이 docstring 한 줄이다.**
선언과 진술과 타입과 `DD:` 표지는 건드리지 않았다.

### 62-2. 변경 내역

받은 지시서는 `IMPLEMENTATION_PLAN_claim_claim-1.md` 이고 짝이 되는 카드는
`ENTRY_CARD_claim_claim-1.json` 이며 그 sha256 이 `ff94b6768ff7d422` 로 시작한다. 구간마다
커밋을 갈랐다.

| 커밋 | 구간 | 무엇을 바꾸었나 |
|---|---|---|
| `e6dfdda` | 1 | `arcs.json` 의 `claim` 레코드를 확인만 하고 `CLAUDE.md` §1-5 의 낡은 지목을 고침. 2 행 |
| `0453dca` | 2 | 카드의 entry 열둘을 원장 셋에 병합하고 등가 검증 뒤 카드 파일 삭제. 233 행 |
| `54b6396` | 3 | 추출 기록 재발행과 docstring 한 줄과 상주 문서 둘. 20 행 |
| (이 커밋) | 4 | `BUILD_LOG.md` §62~§65 와 `CLAUDE.md` §9 의 계수 |

**카드 파일 삭제를 병합과 한 커밋에 담았다.** 앞 회차는 그 둘을 갈랐는데 그때는 한 구간에
되돌릴 수 없는 조작이 둘이면서 아크 종료가 함께 있었다. 이번 구간 2 의 되돌릴 수 없는 조작은
병합과 삭제 둘뿐이고 지시서 부록 B 가 그 둘의 되돌림을 한 항으로 묶어 들었다.

### 62-3. `claim` 아크가 열렸다

`arcs.json` 의 `claim` 레코드는 결정 세션이 호출 전에 써서 `charter` 와 `propagation` 사이에
이미 서 있었고 **집행은 확인만 하고 고치지 않았다.** 지시서가 든 아홉 필드를 전부 맞대어
어긋남이 0 이다.

| 필드 | 값 |
|---|---|
| `discriminator` | 청구권은 누구에게 무엇을 언제 요구하는가 |
| `ph` | `PH-2` |
| `phases` | `claim-1` 부터 `claim-7` 까지 일곱 |
| `status` · `opened` · `closed` | `open` · `2026-09-23` · `null` |
| `holes` | `CF-213` · `CF-579` |
| `cq` | `CQ-fin-1` · `CQ-fin-2` |

**레코드가 먼저 서 있어야 병합이 통과한다.** 검사 2 와 3 이 병합되는 항목의 `origin.arc` 와
`origin.phase` 를 이 레코드와 대조하기 때문이며, 그래서 확인이 병합보다 앞선 구간에 놓였다.
...
`rejected.json` 은 빈 레지스터이고 건드리지 않았다. 차분이 추가 233 행이고 삭제 0 행이라
기존 entry 568 건을 한 글자도 고치지 않았다.

**등가 검증은 열둘 전부 통과했다.** 원장에서 뽑은 entry 와 카드의 대응 entry 를 키를 정렬한
표준형의 바이트로 맞댔고 문자열에는 정규화를 걸지 않았다. 카드의 순서가 각 배열 끝에 그대로
서 있는 것과 `rejected.json` 원문이 바이트로 불변인 것도 함께 확인했으며, 그 통과 뒤에 카드
파일을 지웠다.

### 62-5. 카드의 키 순서가 원장의 관례와 달랐다

**키 집합은 열둘 전부 원장과 정확히 같고 순서만 달랐다.** 카드가 `decisions` 에서
`id · origin · tier · layer · names · statement · basis · related` 를 쓰는데 원장의 기존
511 건은 `id · origin · names · statement · basis · tier · layer · related` 계열을 쓴다.
`deferred` 와 `retirements` 도 같은 형으로 갈렸다.

지시서 OP-3 이 「키 순서는 각 파일의 현행 관례를 따르고」를 들었으므로 **순서만 원장 관례로
돌리고 값은 손대지 않았다.** 관례는 기존 entry 에서 읽었고 `deferred` 33 건과 `retirements`
24 건이 각각 한 가지 순서를, `decisions` 511 건이 `names` 와 `check` 와 `clarifies` 와
`resolves` 의 유무로 갈리는 네 가지 순서를 쓰고 있었다.

**그 결과 등가 검증을 원시 바이트 비교로 할 수 없게 됐다.** `CLAUDE.md` §14 가 등가 검증을
「바이트로 같은지」로 드는데 키 순서를 바꾸면 그 비교가 성립하지 않는다. 키를 정렬한 표준형의
바이트 비교로 갈음했고 문자열은 정규화하지 않았으므로 OP-4 가 든 「문자열은 정규화 없이 같아야
한다」는 그대로 지켰다. **§14 의 문면과 OP-3 의 문면이 곧이곧대로는 양립하지 않는다.**

### 62-6. 사라진 문서를 향한 지목 열하나를 지웠다

이 회차가 고친 문면의 대부분이 **리포에 들어온 적이 없고 지금은 사라진 문서를 향한 지목**을
지우는 것이다. 자리가 넷으로 갈린다.

| 자리 | 무엇을 지웠나 | 계수 |
|---|---|---|
| `CLAUDE.md` §1-5 | 아크 개막 문서 지목. J-30 으로 없어졌고 그 자리를 `arcs.json` 이 받는다 | 2 행 |
| `docs/extractions/extraction-16-plantin-sapra-shin-2007.md` | `ph2-01` 지목. 판정을 그쪽에 맡기던 자리다 | 7 자리 |
| `CrisisFramework/Definition/Constraint.lean` | docstring 의 상위 프로젝트 이름 | 1 행 |
| `docs/reading-queue-v0.md` §1 | 폐기된 `S-5` 지목을 후속 조항 `S-7` 로 | 1 행 |

추출 기록은 재발행 형으로 처분했다. **머리 블록을 앞에 놓아 무엇을 지웠고 원문이 어느 커밋에
있는지를 그 문서 자신이 들게 했고** 절 구조와 절 번호는 그대로 뒀다. 절 제목 22 개의 번호가
전부 불변이며 고친 뒤 `ph2-01` 이 0 회 남았다. §3-1 의 한 문장만 지목을 지우면서 두 문장을
하나로 합쳤고 그 사실도 머리 블록이 든다.

### 62-7. 검사 24 가 세 자리에서 0 자리로 돌아섰다

`CF-576` 이 `S-5` 를 폐기하면서 **상주 문서 셋의 지목이 선언 없는 폐기 지목이 됐다.** 병합만
한 상태에서 검사 24 가 반드시 실패하는 자리이며, 그래서 지시서가 `verify.sh` 를 문서 조작이
끝난 뒤에 돌리라는 순서 제약을 걸었다.

| | 병합 직후 | 구간 3 뒤 |
|---|---|---|
| 선언 없는 폐기 지목 | 3 | **0** |
| 규율 ID 토큰 | 205 | **206** |
| 선언으로 면제된 폐기 지목 | 18 | **19** |
| 미실재 | 0 | **0** |

**처분이 자리마다 달랐다.** `SPEC.md` §3.3 은 문장의 근거 자체를 바꾸어 `S-5` 를 드는 일이
없게 했고, `reading-queue-v0.md` 는 한 자리를 머리의 폐기 지목 선언에 넣어 면제받게 하고 다른
한 자리를 후속 조항 `S-7` 로 갈아 끼웠다. 토큰이 하나 늘고 면제가 하나 는 것이 그 선언에
더한 `S-5` 하나다.

### 62-8. 검사 10 의 실패는 기지의 것이다

검사 10 이 계속 실패하며 잡은 자리가 `docs/phases/charter-20-closure.md` 의 35·134·147 행이
폐기된 `CF-566` 을, 178 행이 폐기된 `CF-1` 을 드는 넷뿐이다. **그 문서는 앞 회차의 역사
기록이라 고치지 않으며 이 회차의 조작과 무관하다.** 지시서 부록 A 가 그 넷을 미리 들었고
「그 넷 말고 다른 자리가 걸리면 멈춘다」를 걸었는데, 실측이 정확히 그 넷이고 더 늘지 않았다.

**검사 10 의 산문 유니버스는 `docs/` 아래의 `.md` 전량뿐이다.** `BUILD_LOG.md` 는 리포 루트에
있어 그 유니버스 밖이며, 이 절이 폐기된 `CF-310` 과 `CF-116` 을 드는 것이 그 검사를 깨뜨리지
않는다. 검사 24 의 유니버스 밖이기도 하다.

### 62-9. 새 항목 열둘의 참조 방향

**열둘 전부가 자기보다 앞선 번호만 가리킨다.** 뒤를 가리키는 자리가 0 건이다.

| 항목 | 가리키는 것 |
|---|---|
| `CF-569` | `CF-140` |
| `CF-570` | `CF-310`(폐기 대상) · `CF-569` |
| `CF-571` | `CF-117` |
| `CF-572` | `CF-571` |
| `CF-573` | `CF-117` · `CF-571` |
| `CF-574` | `CF-293` |
| `CF-575` | 없음 |
| `CF-576` | `CF-116`(폐기 대상) · `CF-575` |
| `CF-577` | `CF-41` · `CF-42` · `CF-71` · `CF-175` · `CF-213`~`CF-218` · `CF-246` · `CF-574`~`CF-576` |
| `CF-578` | `CF-147` |
| `CF-579` | `CF-42` · `CF-213` |
| `CF-580` | `CF-213` · `CF-571` · `CF-573` · `CF-579` |

**폐기된 `CF-310` 과 `CF-116` 을 드는 자리는 리포 전체에서 넷뿐이고 전부 폐기 레코드 자신의
`target` 과 `statement` 다.** `decisions.json` 안에서 그 둘이 나오는 자리는 자기 자신의 `id`
둘뿐이며, 살아 있는 것처럼 드는 자리가 새로 생기지 않았다. 검사 10 이 그것을 리포 전체에서
독립으로 확인했다.
...
| 시점 | `OUTPUT_DIGEST` |
|---|---|
| 구간 3 (문서 조작 뒤) | `21cd8c4dc856` |
| 구간 4 (`CLAUDE.md` §9 뒤) | `21cd8c4dc856` |

앞 회차의 `be44a7d9cffc` 에서 움직였다. **움직인 것은 구간 3 이고 구간 4 는 움직이지 않았다.**
구간 3 이 검사 24 를 실패에서 통과로 돌리고 그 계수 셋을 바꾸었으며 검사 10 이 잡는 자리를
일곱에서 넷으로 줄였다.

**`CLAUDE.md` §9 의 갱신이 값을 움직이지 않는 것은 앞 회차와 같은 자리다.** 그 파일이 검사
24 의 유니버스에 들지만 표의 숫자를 바꾸는 것이 규율 ID 토큰을 더하지도 줄이지도 않는다. 앞
회차가 같은 관측을 냈고 이번에는 그것이 기대값이었으므로 어긋남이 아니다.

구간 4 의 두 판을 맞댄 차분은 `step2-build` 의 진행 표시 세 줄뿐이고 그것이 빌드 job 의 진행
계수와 소요 시간이다. `Build completed successfully (978 jobs)` 은 두 판이 같다.

### 62-11. 미전사분

**이 회차의 미전사분은 0 건이다.** 62-5 가 드는 키 순서의 어긋남이 이 회차가 낸 유일한
결손인데 그것이 사는 자리가 지시서와 카드이지 리포의 추적 파일이 아니다. `CLAUDE.md` §7-2 의
표가 그 부류를 미전사분에서 제외한다.

**회차를 건너 살아남은 결함도 없다.** 앞 판의 값이 새 판에 그대로 남은 자리가 없다.

---

## 63. 해석 (관측값 아님)

- **지목은 대상이 사라져도 남는다.** 이 회차가 지운 열한 자리가 전부 그 형이며, 가리키던 문서가
  없어졌는데 가리키는 쪽이 그대로 서 있었다. 검증기가 그 가운데 셋만 잡았고 그 셋은 규율 ID 가
  걸려 있어서 잡혔다. **나머지 여덟은 ID 가 걸리지 않은 산문이라 그물 밖이었고 사람이 읽어야만
  드러났다.** `CF-348` 이 든 「ID 가 그물이고 ID 없는 문면이 그 그물 밖」과 같은 형이 문서 축에서
  다시 난 셈이다.
- **순서 제약이 이 회차의 성격을 드러낸다.** 병합이 검사 24 를 반드시 실패시키고 문서 조작이
  그것을 되돌리는 구조라 중간 상태가 검증을 통과하지 못하는 것이 설계였다. **낙착을 싣는 것과 그
  낙착이 죽인 조항을 드는 자리를 치우는 것이 한 페이즈 안에서 갈라져 있다**는 뜻이며, 구간을
  나누지 않았으면 어느 실패가 기대된 것인지 가려내지 못했을 것이다.
- **재발행이 수정과 다른 처분이다.** 추출 기록은 원장이 아니라 고칠 수 있는 문서인데 그래도
  머리 블록을 놓아 무엇을 지웠고 원문이 어디에 있는지를 남겼다. **고칠 수 있는 문서에도 고친
  자취를 남기는 쪽을 골랐고** 그 근거가 원문을 든 커밋 하나뿐이면 그 커밋을 찾는 수단이 문서
  밖에만 있게 되기 때문이다.
- **키 순서의 어긋남은 두 문면이 각각 옳아서 생겼다.** OP-3 이 관례를 따르라 하고 §14 가 바이트
  비교를 요구하는데 카드와 원장의 순서가 같았으면 둘이 충돌하지 않았다. **어긋남의 원인이 조항이
  아니라 카드를 만드는 절차에 있다.**

---

## 64. 진술을 바꿔야만 통과할 것 같은 지점

**없다.** 이 회차는 `theorem` 과 `def` 를 하나도 세우지 않았고 Lean 파일의 변경이 docstring
한 줄뿐이다. 진술과 타입과 시그니처와 `DD:` 표지를 건드리지 않았고 `sorry` 를 둔 자리가 없다.

**검증기와 CI 워크플로를 고쳐 통과시킨 자리도 없다.** 검사 24 가 실패에서 통과로 돌아선 것은
지시서가 지시한 문서 조작의 결과이고 검사 자신을 고친 것이 아니다. 검사 10 의 실패는 고치지
않고 그대로 뒀으며, 그 넷이 역사 기록이라 고치는 것이 곧 기록을 지우는 것이기 때문이다.

---

## 65. 사람의 판단이 필요해 보이는 것 (실행하지 않음)

1. **카드의 키 순서가 원장의 관례와 다르다.** 62-5 가 그 자리를 든다. 카드를 만드는 쪽이 원장의
   현행 순서를 읽어 맞추면 OP-3 의 재배열이 필요 없어지고 §14 의 바이트 비교가 그대로 성립한다.
2. **`CLAUDE.md` §14 의 「바이트로 같은지」가 키 순서를 재배열하는 병합과 양립하지 않는다.** 앞
   항의 절차를 고치든 이 조항의 문면을 고치든 둘 가운데 하나가 필요해 보인다.
3. **검사 10 이 `docs/phases/` 의 역사 기록을 계속 잡는다.** 넷이 남아 있고 고칠 수 없는 부류인데
   면제 수단이 없다. 상주 문서에는 머리의 폐기 지목 선언이 그 수단인데 `docs/phases/` 에는 그것이
   없다.
4. **검사 10 과 검사 24 의 유니버스가 다르다.** 검사 10 이 `docs/` 아래의 `.md` 전량을 훑고 검사
   24 는 상주 문서 여섯과 Lean 셋만 본다. `docs/phases/` 와 `docs/extractions/` 가 한쪽에는 들고
   다른 쪽에는 들지 않는데 그 차이를 드는 조항이 원장에 없다.
5. **사라진 문서를 향한 지목을 재는 검사가 없다.** 이 회차가 지운 열한 자리 가운데 여덟이 ID 가
   걸리지 않은 산문이라 어느 검사에도 잡히지 않았다.
6. **검사 16 이 `artifact` 발화 여섯을 계속 보고한다.** 앞 회차들과 같은 자리이고 읽는 쪽이 없다.
   이번에 `CF-579` 의 `blocking` 발화가 하나 더 들어 일곱이 됐다.
7. **`OUTPUT_DIGEST` 의 차분에 쓸 전판의 출력을 보존할 자리가 없다.** 앞 회차와 같은 자리다.
   이번에는 집행이 스크래치에 두 판을 남겨 맞댔으나 그것이 커밋에 남지 않는다.

---

## 66. `claim-2` — 관측값

검증일: 2026-09-24. 이 회차는 `claim` 아크의 페이즈 2이며, **`CQ-fin` 열하나를 관측층 Lean
선언으로 고정하고 그 고정 절과 대조 설비와 `cq` 필드를 읽는 검사를 세우는 것**이 판별선이다.
**관측층이 이 리포에 처음 섰다.**

### 66-1. 빌드

네 파일 전량이 통과했고 `sorry` 와 실패가 없다. `Build completed successfully (979 jobs)` 이며
error 0 warning 0 이다. 늘어난 1 job 은 새 파일 `CrisisFramework/Observation/CompetencyQuestion.lean`
이다.

**관측층 파일이 지시서 부록 B 그대로 한 번에 컴파일됐다.** 결정 세션의 환경에 Lean 툴체인이
없어 한 번도 컴파일된 적이 없는 파일이었으나, `deriving` 절도 import 도 고치지 않았다.
`ObservedValue` 의 `deriving DecidableEq, Repr` 도 이 핀에서 그대로 섰다.

### 66-2. 변경 내역

받은 커밋은 `18ca55a` 이다. 구간마다 커밋을 하나씩 두었다.

| 커밋 | 구간 | 카드 | 무엇을 바꾸었나 |
|---|---|---|---|
| `6f69383` | 1 | K1 | 카드의 결정 entry 열셋을 `decisions.json` 끝에 병합 |
| `49009c6` | 2 | K2~K6 | 카드 파일 삭제, 검사 10·16·21 의 수정, `verify.sh` 의 `TARGETS` 훑기 |
| `55f890c` | 3 | K7~K10 | 관측층 파일, 추출 기록 E-17, 관측층 프로브, 읽기 대기열 §1 |
| `664a9d5` | 4 | K11~K14 · K18 · K10-a | 관측층 기준선과 공리 감사, CQ 고정 절과 `step7-cqfin`, 검사 28·29·30, 읽기 대기열 §5 |
| (이 커밋) | 5 | K19 · K16 · K15 | 낡은 계수와 서술 다섯 자리, `CLAUDE.md` 셋, 이 절 |

**K18 · K19 · K10-a 는 지시서에 없고 정지점 프롬프트가 더한 카드다.** 각 구간의 보고가 든 결손을
결정 세션이 받아 다음 구간의 카드로 내렸다.

### 66-3. 원장 병합과 등가 검증

| 레지스터 | 병합 전 | 카드 | 병합 후 |
|---|---|---|---|
| decisions | 520 | 13 | **533** |
| deferred | 34 | 0 | **34** |
| retirements | 26 | 0 | **26** |
| rejected | 0 | 0 | **0** |
| 합 | 580 | 13 | **593** |

병합 전 `decisions.json` 의 sha256 은 `75bacdcba7669f13…` 이고 병합 후는 `ccc394603d087b7a…` 다.
차분은 추가 209 행이고 삭제 0 행이다. 검사 19 는 `CF-1~CF-593` 에 결번 0 건을 낸다.

**등가 검증을 두 번 했고 두 값이 같았다.** 병합된 배열의 마지막 열셋과 카드의 `new.decisions` 를
`json.dumps(ensure_ascii=False, indent=1)` 로 직렬화해 바이트로 맞댔다. 구간 1 의 끝과 구간 2 의
카드 삭제 앞에서 둘 다 `ef4d4123a2cfec4d…` 를 냈다. **카드의 키 순서가 원장의 관례와 같아서 재배열
없이 원시 바이트 대조가 성립했다.** 이것이 `CF-591` 이 고치려던 자리이며, 앞 회차의 62-5 가 든
결손이 이번 카드에서는 나지 않았다.
...
| 대상 | 실측 |
|---|---|
| `observationProbe` | 무의존 |
| `observationProbeProd` (`Prod`) | 무의존. 관측층 실효 기준선 `[]` |
| 관측층 선언 27 | 전부 무의존 |
| 파생 인스턴스 8 (`DecidableEq` 넷 · `Repr` 넷) | 전부 무의존. 스크래치에서 따로 찍었고 `verify.sh` 의 대상은 아니다 |

`step4-baseline` 의 감사 대상이 15 에서 **42** 로 늘었다(회계층 5 · 용어집 4 · 정의층 6 · 관측층
27). 늘어난 27 은 K18 이 관측층 선언을 `audit_layer` 에 더한 것이며, 이름을 박지 않고 관측층
파일에서 규칙으로 뽑았다. **기준선 초과분은 0 이다.** 분류할 대상이 없다는 것을 결정 세션이
확정했다.

`docs/baseline.md` §2 의 관측층 행을 「미측정」에서 `[]` 로 채웠다. 측정 시점 칸에는
`claim-2` (2026-09-24) 를 적었으며, 앞 행들의 「N차 세션」 같은 상대 지목을 쓰지 않았다.

### 66-5. 구간마다의 `verify.sh`

| 구간 | 결과 | 실패 | `OUTPUT_DIGEST` |
|---|---|---|---|
| 받은 커밋 | 실패 | 검사 10 | `21cd8c4dc856` (앞 회차 62-10) |
| 1 | 지시서에 따라 돌리지 않았다 | | |
| 2 | 실패 | 검사 7 | `4c195327a898` |
| 3 | 실패 | 검사 7 | `01c285fa5c69` |
| 4 | **전량 통과** | 없음 | `f19316d6b387` |
| 5 | **전량 통과** | 없음 | `f19316d6b387` |

**기지의 실패 둘이 이렇게 풀렸다.**

- 검사 10 은 받은 커밋부터 `docs/phases/charter-20-closure.md` 의 네 자리에서 실패했고, 구간 2 의
  K3 이 역사 기록을 유니버스에서 빼면서 풀렸다(`CF-587`).
- 검사 7 은 구간 1 의 병합으로 `CF-585` 가 원장에 들어서면서 실패하기 시작했다. 그 `check` 가 드는
  `step7-cqfin` 이 구간 4 의 K13 에서 서면서 풀렸다.

**구간 5 가 값을 움직이지 않았다.** 이 구간은 `SPEC.md` 와 `verify.sh` 머리 주석과
`check_wiki.py` 의 docstring 과 help 와 `CLAUDE.md` 와 이 파일만 고쳤고, 그 어느 것도 `verify.sh`
의 출력에 나지 않는다. `CLAUDE.md` 는 검사 24 의 유니버스에 들지만 더한 문면에 규율 ID 토큰이
없다.

구간 4 에서 검사 계수가 27 에서 30 으로, 수행 계수가 25 에서 27 로 늘었다. 늘어난 셋은 K14 가
더한 28 · 29 · 30 이다. 검사 16 의 발화는 구간 3 에서 6 에서 9 로 늘었으며, 늘어난 셋은 트리거가
`CrisisFramework/Observation/` 을 가리키는 `CF-151`(K-8) · `CF-181`(H-28) · `CF-209`(H-56) 이다.
K7 이 그 디렉터리를 세운 순간 깨어났다. 구간 2 에서는 K4 가 `blocking` 가지를 빼서 `CF-579` 의
발화 하나가 빠졌다.

### 66-6. 음성 대조

모두 리포 밖 사본에서 했고 확인 뒤에 사본을 지웠다.

| 대상 | 주입 | 결과 |
|---|---|---|
| 검사 10 | 역사 기록이 아닌 `docs/reading-queue-v0.md` 에 폐기된 id | 실패 |
| 검사 10 | `docs/phases/` 의 역사 기록에 폐기된 id | 통과(유니버스 밖) |
| `step3-forbidden` | 관측층 파일 사본에 `noncomputable` · `sorry` | 두 항목이 실패 |
| `step3-forbidden` | 관측층 파일 사본에 `ℝ` | 층별 `ℝ` 금지가 실패 |
| 검사 28 | `cq` 에 고정 절에 없는 `CQ-fin-12` | 실패 |
| 검사 28 | `cq` 에 형식이 아닌 값 | 실패 |
| 검사 28 | `CQFIN` 블록을 비움 | 실패 |
| 검사 29 | 처분 항목이 없는 아크에 `CQ-fin-7` 을 걸고 `--arc-close` | 실패 |
| `step7-cqfin` | docstring 한 글자 변경 | 실패(다이제스트가 달라짐) |
| `step7-cqfin` | 답 함수 타입에 인자 하나 추가 | 실패 |
| `step7-cqfin` | 고정 절 두 줄의 순서를 바꿈 | 실패(순서까지 대조함) |
| `step7-cqfin` | docstring 줄 끝 공백만 추가 | **통과.** 추출 규칙 3항이 줄 끝 공백을 지운다는 양성 대조 |

`step3-forbidden` 은 수치 리터럴과 구체 수 타입을 보지 않는다. 그래서 관측층 파일의 주석을 지운
코드를 손으로 쟀고, 숫자와 `ℕℤℚℝ` 와 `Nat`·`Int`·`Rat`·`Real`·`Float`·`Fin` 과 `import` 가
전부 0 이었다.

### 66-7. 미전사분

**결정 세션이 승인한 문면 넷이 원장에 아직 없다.** 이 넷은 구간 4 에서 카드에 문면이 없어 집행이
쓴 것이다.

- `SPEC.md` §2.1 의 28행 가운데 「잡는 실패」 칸과 「주입 시험」 칸
- `SPEC.md` §2.2 의 30행 가운데 「왜 실패가 아닌가」 칸
- `docs/baseline.md` §6 의 「고정 절의 값은 §5 의 `CQFIN` 블록이 든다」

리포의 추적 파일에 사는 문면이므로 `CLAUDE.md` §7-2 에 따라 미전사분이다. 결정 세션이 승인을
`claim-3` 에서 원장에 싣는다고 밝혔다.

**회차를 건너 살아남은 결함이 하나 있다.** `check_wiki.py` 모듈 docstring 의 검사 계수는 이
회차 전부터 「열여덟」으로 낡아 있었고 `SPEC.md` 는 「열아홉」을 들었다. 이 회차의 K19 가 고쳤다.

### 66-8. 해시

| 시점 | `OUTPUT_DIGEST` |
|---|---|
| 구간 5 (K19 · K16 뒤, 이 절을 적기 전) | `f19316d6b387` |
| 구간 5 (이 절을 적은 뒤) | `f19316d6b387` |

구간 4 의 값과 같다. `verify.sh` 는 이 파일을 읽지 않으므로 이 절을 적기 전과 뒤의 두 값이
같아야 하며, 실제로 같았다. 구간 4 의 판과 이 구간의 판을 맞댄 차분은 `step2-build` 의 진행 표시
네 줄뿐이고 그것이 빌드 job 의 진행 계수와 소요 시간이다.

---

## 67. 해석 (관측값 아님)

- **검사 29 가 산문 대조로는 실패할 수 없다.** 아크를 열 때 그 개막 결정이 자기 CQ 를 이름으로
  적으므로, `origin.arc` 가 그 아크인 항목 가운데 CQ 를 드는 것이 언제나 하나 이상 있다. 실제로
  `claim` 을 모의로 닫아 돌려 보니 검사 29 가 처분 없이 통과했다. `CQ-fin-1` 은 `CF-572` ·
  `CF-579` · `CF-580` 에, `CQ-fin-2` 는 `CF-580` 에 나온다. **판정이 필드가 아니라 산문에
  얹혔다**는 형이며, 처분은 `claim-3` 의 몫으로 넘어갔다.
- **처음 컴파일되는 파일이 한 번에 섰다는 것은 무엇이 확인됐는지를 좁혀 읽어야 한다.** 확인된
  것은 서명과 타입이 성립한다는 것이며, 답 함수의 본문은 아직 witness 뿐이다. 구멍은 뒤의 아크가
  본문을 채울 때 드러난다.
- **카드가 지시한 문면과 사실에 맞는 문면이 갈린 자리가 여럿이었다.** 검사 10 의 유니버스를 든
  자리가 표의 행이 아니라 문단이었고, step6 에 딸린 연속 줄을 카드가 세지 않았다. 또 계수를 고치는
  카드가 같은 계수를 든 다른 자리를 세지 않았다. 셋 다 조작을 바꾸지 않는 결손이었다.

---

## 68. 진술을 바꿔야만 통과할 것 같은 지점

**없다.** 관측층 파일의 진술과 타입과 docstring 이 부록 B 와 한 글자도 다르지 않은 채
컴파일됐다. 이 회차가 세운 Lean 선언에는 `theorem` 과 `example` 이 없어서 새 변이 대상도 없다.

**검증기와 CI 워크플로를 고쳐 통과시킨 자리도 없다.** 검사 7 과 10 이 통과로 돌아선 것은
지시서가 지시한 조작의 결과다.

---

## 69. 사람의 판단이 필요해 보이는 것 (실행하지 않음)

1. **검사 29 의 판정을 필드로 옮기는 일.** 67 의 첫 항이 그 자리다. 결정 세션이 `claim-3` 에서
   처분한다고 밝혔다.
2. **`check_wiki.py` 모듈 docstring 의 「않으면 둘은 수행되지 않았음을」.** K19 의 조작 4 가
   같은 문장의 앞을 「20 과 21 과 29 를」로 바꾸었으나, 뒤의 「둘은」은 어느 카드도 지목하지 않아
   그대로 남았다.
3. **witness 의 docstring 이 D-2 의 3항목을 채우지 않는다.** `CF-593` 이 드는 대로 어긋난 자리가
   셋에서 열넷이 됐다.
